<template>
  ...
  >
    <h1 class="text-2xl md:text-3xl lg:text-4xl font-bold uppercase">speakers</h1>
  </div>
  <div class="px-6 md:px-20 lg:px-40 flex flex-col gap-16 md:gap-24 lg:gap-32 mb-16 md:mb-24 lg:mb-40">
    <ProductCard
      v-for="(product, index) in speakers"
      :key="product.id"
      :product="product"
      :index="index"
...
// Get only speakers and sort by newest first
const speakers = computed(() => {
  return getProductsByCategory("speakers").sort((a, b) => {
